    ...
    companion object {
        ...
        private fun adaptForLevel3(session: Session) : ProgramResult {
            ...
            for (i in session.program.exercises.indices)
            {
                ...
                var repetitions = session.seriesRepetitions.subList(offset, seriesCount)
        ...
        private fun adaptForLevel1(session: Session) : ProgramResult{
            ...
            if(TimeUnit.DAYS.convert(LocalDateTime.now().toEpochSecond(ZoneOffset.UTC) - session.dateOfProgramBegining.toEpochSecond(ZoneOffset.UTC), TimeUnit.SECONDS) / 7 >= 2 && session.program.index == 0)
            {
            ...
            if (TimeUnit.DAYS.convert(LocalDateTime.now().toEpochSecond(ZoneOffset.UTC) - session.dateOfProgramBegining.toEpochSecond(ZoneOffset.UTC), TimeUnit.SECONDS) / 7 >= 3 && session.program.index == 1)
            {
                result.program = FirebaseService.programs.find {p -> p.level == session.program.level + 1}!!
                result.levelChanged = true
                return result
